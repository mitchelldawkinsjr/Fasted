...
# Installs to /opt/360ws/clients/docker-app/supabase and joins 360ws-network.
set -euo pipefail
...
API_URL="${API_URL:-https://api.fasted.360web.cloud}"
...
# Avoid host port conflicts (360ws-api uses 8000). NPM reaches Kong via docker network on container port 8000.
...
echo "  3. Update fasted-calendar .env with VITE_SUPABASE_URL=https://api.fasted.360web.cloud and VITE_SUPABASE_ANON_KEY"
