#!/usr/bin/env bash
# Repoint an existing API proxy host from PocketBase to Supabase Kong.
set -euo pipefail
...
HOST_ID=$(sudo sqlite3 "$DB" "SELECT id FROM proxy_host WHERE domain_names LIKE '%${API_DOMAIN}%' AND is_deleted=0 LIMIT 1;" || true)

if [ -z "$HOST_ID" ]; then
  echo "No proxy host found for ${API_DOMAIN}. Run scripts/npm-add-fasted.sh first or add manually in NPM."
  exit 1
fi
...
echo "Done. ${API_DOMAIN} now forwards to ${FORWARD_HOST}:${FORWARD_PORT}"
