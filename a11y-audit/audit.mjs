#!/usr/bin/env node
/**
 * WCAG accessibility audit via axe-core for the Fasted Calendar PWA.
 */
...
const PAGES = [
  ...
  { name: "journal", url: `${BASE}/journal`, path: "/journal" },
  { name: "progress", url: `${BASE}/progress`, path: "/progress" },
  { name: "mood", url: `${BASE}/progress/mood`, path: "/progress/mood" },
  { name: "phases", url: `${BASE}/phases`, path: "/phases" },
  { name: "settings", url: `${BASE}/settings`, path: "/settings" },
];
...
function aggregateReports(reports, label) {
  ...
  const sorted = Object.values(summary).sort(
    (a, b) =>
      severityWeight(b.impact) - severityWeight(a.impact) ||
...
async function main() {
  ...
  if (aggregate.totalViolations > 0) {
    process.exitCode = 1;
  }
}
